...
- order/order/created TODO link: Used to import `Order` and associated `OrderItem`s from the `Order` bounded context.
- [catalog/product-variant-version/created](/docs/docs/dev-manuals/services/catalog#product-variant-versioncreated): Used to import `ProductVariantVersion` from the `Product` bounded context.
