...
<CustomImage path="/renderedDiagrams/invoiceDomainModel" width="1365" height="131" />
...
The service provides a GraphQL API, which is documented in the [GraphQL API documentation](/docs/graphql/invoice).
...
<div className="repository-structure">
 ...
 `http_event_service.rs` Handling of incoming events - `invoice.rs` Invoice datatype - `main.rs` Service execution and
 schema generation - `order.rs` Order datatype - `query.rs` GraphQL queries and associated functions
</div>
