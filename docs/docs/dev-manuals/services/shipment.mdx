...
The service provides a GraphQL API, which is documented in the GraphQL API documentation [GraphQL API documentation](/docs/graphql/shipment).
...
- payment/payment/payment-enabled: Used to create new shipments for an order when the order and associated payment is validated successfully.
