...
<div className="repository-structure">
 ...
 `base_connection.rs` Generic GraphQL connection - `http_event_service.rs` Handling of incoming events - `main.rs`
 Service execution and schema generation - `mutation_input_structs.rs` GraphQL input datatypes for mutations -
 `mutation.rs` GraphQL mutations and associated functions - `order_datatypes.rs` GraphQL datatypes for connection
 element ordering - `product_variant_connection.rs` GraphQL connection of product variants - `query.rs` GraphQL queries
 and associated functions - `review_connection.rs` GraphQL connection of review - `review.rs` Review datatype -
