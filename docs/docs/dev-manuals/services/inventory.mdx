...
The service provides a GraphQL API, which is documented in the [GraphQL API documentation](/docs/graphql/payment).
...
## Business Processes
The inventory service is part of the [Buy](/docs/docs/dev-manuals/architecture/business-processes.md#buy-process) and [Return](/docs/docs/dev-manuals/architecture/business-processes.md#return-process) process.
...
#### inventory/product-item/release-succeeded

This event is published after an order compensation event. It indicates, that all previously reserved `ProductItems` for the `ProductVariants` of an `Orders` `OrderItems` were released again. 
